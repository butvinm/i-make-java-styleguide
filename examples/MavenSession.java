...
public class MavenSession implements Cloneable {
     ...
     * session. This is also the potentially constrained set of projects by
     * using --projects on the command line.
     */
    ...
    /**
     * Plugin context keyed by project ({@link MavenProject#getId()}) and by
     * plugin lookup key ({@link PluginDescriptor#getPluginLookupKey()}). Plugin
     * contexts itself are mappings of {@link String} keys to {@link Object}
     * values.
     */
    @SuppressWarnings("checkstyle:linelength")
    private final ConcurrentMap<String, ConcurrentMap<String, ConcurrentMap<String, Object>>> pluginContextsByProjectAndPluginKey = new ConcurrentHashMap<>();
    ...
    public void setProjects(List<MavenProject> projects) {
        if (!projects.isEmpty()) {
            ...
            this.topLevelProject = projects.stream()
                .filter(project -> project.isExecutionRoot()).findFirst()
                .orElse(first);
        } else {
     ...
     * The user properties have been configured directly by the user on his
     * discretion, e.g. via the {@code -Dkey=value} parameter on the command
     * line.
     *
     ...
     * activation. The system properties are collected from the runtime
    ...
    public ProjectBuildingRequest getProjectBuildingRequest() {
        return request.getProjectBuildingRequest()
            .setRepositorySession(getRepositorySession());
    }
    ...
    /**
     * Returns the plugin context for given key
     * ({@link PluginDescriptor#getPluginLookupKey()} and {@link MavenProject},
     * never returns {@code null} as if context not present, creates it.
     *
     * <strong>Implementation note:</strong> while this method return type is
     * {@link Map}, the returned map instance implements {@link ConcurrentMap}
     * as well.
     *
     */
    ...
    public MavenSession clone() {
        try {
            ...
            // the default must become the current project of the thread that
            // clones this
            MavenProject current = getCurrentProject();
            // we replace the thread local of the clone to prevent write through
            // and enforce the new default value
    ...
    public MavenSession(PlexusContainer container,
        MavenExecutionRequest request, MavenExecutionResult result,
        MavenProject project) {
        this(container, request, result,
            Arrays.asList(new MavenProject[] { project }));
    }
    ...
    public MavenSession(PlexusContainer container, Settings settings,
        ArtifactRepository localRepository, EventDispatcher eventDispatcher,
        ReactorManager unused, List<String> goals, String executionRootDir,
        ...
        this.request.setBaseDirectory(
            (executionRootDir != null) ? new File(executionRootDir) : null);
    ...
    /**
     * Adapt a {@link MavenExecutionRequest} to a {@link Settings} object for
     * use in the Maven core. We want to make sure that what is ask for in the
     * execution request overrides what is in the settings. The CLI feeds into
     * an execution request so if a particular value is present in the execution
     * request then we will take that over the value coming from the user
     * settings.
     */
    private static Settings adaptSettings(MavenExecutionRequest request) {
        ...
        return new Settings(org.apache.maven.api.settings.Settings.newBuilder()
            .localRepository(
                localRepo != null ? localRepo.getAbsolutePath() : null)
            .interactiveMode(request.isInteractiveMode())
            .offline(request.isOffline())
            .proxies(request.getProxies().stream().map(Proxy::getDelegate)
                .collect(Collectors.toList()))
            .servers(request.getServers().stream().map(Server::getDelegate)
                .collect(Collectors.toList()))
            .mirrors(request.getMirrors().stream().map(Mirror::getDelegate)
                .collect(Collectors.toList()))
            .profiles(request.getProfiles().stream().map(Profile::getDelegate)
    ...
    @Deprecated
    //
    // Used by Tycho and will break users and force them to upgrade to Maven 3.1
    // so we should really leave
